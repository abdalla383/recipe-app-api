...
def test_create_recipe(self):
    ...
    payload = {
        'title': 'Sample recipe',              # ✅ fixed spelling
        'time_minutes': 30,                    # ✅ fixed spelling
        'price': Decimal('5.99'),
    }
    res = self.client.post(RECIPES_URL, payload)

    print("DEBUG RESPONSE:", res.data)        # Optional: debug print

    self.assertEqual(res.status_code, status.HTTP_201_CREATED)
    recipe = Recipe.objects.get(id=res.data['id'])
    for k, v in payload.items():              # ✅ fixed `.items()`
